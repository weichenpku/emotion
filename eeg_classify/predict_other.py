...
black_idx = [5,6,13,16,25]  #[13] #[5,6,13,16,25,29,36]  #
...
flag_black = False #False
flag_use_loo = False #False#
user_del_list = []
...
def parse_npy_data(SIGNAL_TYPE):
    ...
    #data = np.delete(data, [4,5], axis=1)
...
def predict(train, test, epoch_num, batch_num, user):
    ...
    model.fit(X, dummy_y, epochs=epoch_num, batch_size=batch_num, verbose=1)
...
if __name__ == "__main__":
    ...
    if (1): #for feaidx in range(pca_comp_num):
        ...
        for idx in range(repeat_time*USERS):
            ...
            testnum = 1
            experiment = 6
            ret = predict(eeg[0:trainnum*experiment ,:], eeg[trainnum*experiment:, :], 100, USERS-1, rd_eeg[trainnum:])   
            ...
            acc1 = (eeg_classify_result[0][0] + eeg_classify_result[1][1] + eeg_classify_result[2][2])/(experiment*(idx+1))
